...
def content_filter(response: str) -> dict:
    ...
    PII_PATTERNS = {
        ...
        "password": r"(?:password|mật khẩu|mat khau)\s*[:=]\s*\S+",
        "db_connection": r"db\.[a-zA-Z0-9.-]+:\d+",
    }
...
def llm_safety_check(response_text: str) -> dict:
    ...
    try:
        verdict = chat_with_openai(SAFETY_JUDGE_INSTRUCTION, prompt)
        is_safe = "SAFE" in verdict.upper() and "UNSAFE" not in verdict.upper()
        return {"safe": is_safe, "verdict": verdict.strip()}
    except Exception as e:
        return {"safe": True, "verdict": f"Judge error: {str(e)}"}
...
class OutputGuardrailPlugin:
    ...
    def check(self, response_text: str) -> tuple[bool, str]:
        ...
        if self.use_llm_judge:
            judge_result = llm_safety_check(text_to_check)
            if not judge_result["safe"]:
                self.blocked_count += 1
                return False, "Response blocked for safety reasons."
